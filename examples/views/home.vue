  ...
  <div class="explain-container">
    ...
    <div class="flex explain">
      <div class="flex-1 explain-total">
        <p>Infinite-UI</p>
        <p>桌面端组件库</p>
        <p class="last">一套基于 Vue 2.0 ，为平安内部前端开发者提供的<br>桌面端组件库，旨在打造敏捷迭代的开发体验</p>
        <el-button class="btn" @click="toPath()">立即体验</el-button>
      </div>
      <img src='@/assets/img_header.png' alt="桌面端组件库" class="img-ui">
    </div>
    <!--内容模块-->
    <section v-for="(item,i) in explainArray" :key='i' :class="'list'+ i" class="explain-list">
       <div class="explain" :class="{'flex': item.isFlex}">
          <img v-if="item.imgLeft" :src="item.imgSrc" :alt="item.title" :class="'img'+ i" :style="{width: item.width}">
          <div :class="{'flex-1': item.isFlex}" class="explain-txt">
            ...
            { 'tags': item.isTags },
            { 'mb-32': item.isFlex },
            { 'mb-16': !item.isFlex }]">{{item.title}}</p>
            <p class="fs-18"
            :class="[
            {'word-warp':item.isWarp },
            { 'gray': item.titleCl === 'black' },
            { 'white': item.titleCl === 'white' }]">{{item.txt}}</p>
            <el-button v-if="item.isBtn" class="btn" @click="toPath()">立即体验</el-button>
          </div>
          <img v-if="item.imgRight" :src="item.imgSrc" :alt="item.title" :class="'img'+ i" :style="{width: item.width}">
          <!--内容模块第一部分的特殊内容-->
          <div v-if="item.model" class="explain-base">
            <div class="explain-base-list" v-for="(item,i) in baseList" :key='i'>
              <img :src="item.imgSrc" :alt="item.title" :style="{width: item.width}">
              <p class="base-title">{{item.title}}</p>
              <p class="base-txt">{{item.txt}}</p>
              <el-button :disabled="!item.toPath"  @click="toPath()">{{item.tips}}</el-button>
            </div>
          </div>
       </div>
    </section>
    ...
    return {
      ...
      explainArray: [ // 内容模块数据
        ...
        },
        {
          ...
          titleCl: 'black',
          isTags: true,
          txt: '带你了解栅格布局、字体规范、基础控件等设计指南，快速搭建高保真视觉稿，提升产品设计效率。',
          isWarp: true,
          imgRight: true,
        ...
        },
        {
          ...
          titleCl: 'black',
          isTags: true,
          txt: '基于平安项目的矢量图标库。实时预览编辑、一键定制，支持图标源文件下载。',
          isWarp: true,
          imgLeft: true,
        ...
        },
        {
          ...
          titleCl: 'white',
          isTags: true,
          txt: 'Vslab全新数据可视化解决方案，从数据出发，从功能角度对常见图表进行分类，提供规范使用指引。',
          isWarp: true,
          imgRight: true,
...
.btn{
  ...
  &:hover{
    color: #FFF;
    &:hover{
        box-shadow: -1px 2px 8px 3px rgba(255,145,99,.43);
    }
  }
}
.word-warp{
...
.explain-base{
  display: flex;
  background-color: #fff;
  box-shadow: 0 2px 50px 0 rgba(239,120,86,0.20);
  border-radius: 8px;
  margin-top:80px;
  &-list{
    flex: 1;
